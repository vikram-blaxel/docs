...
You are charged for CPU/memory and storage while a sandbox is in active mode.

### Standby mode

An idle WebSocket/TCP connection to a sandbox with no traffic will timeout after 15 minutes (also called _scale-to-zero_). When this happens, Blaxel automatically creates a snapshot of the entire state (including the complete file system in memory, preserving both files and running processes) and transitions the sandbox to standby mode within approximately 5 seconds.
...
You are not charged for CPU/memory while a sandbox is in standby mode. However, you are charged for the storage of the snapshot and/or the volumes.
