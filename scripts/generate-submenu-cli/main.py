...
def collect_generated_pages(generated_dir: str) -> list[str]:
    """
    Scan generated_dir, collect *.md files (minus extension),
    apply EXCLUDE_FILENAMES, and prefix with PAGES_PREFIX.
    """
    pages: list[str] = []
...
def update_docs_json(docs_json_path: str, generated_pages: list[str]) -> None:
    """
    Update docs.json file:

    navigation.tabs[]
      -> tab == "CLI Reference"
        -> groups[] with group == "Overview"
          -> pages[] item with group == "Commands"
             -> replace its pages with generated_pages
    """
    ...
    # Find the "Overview" group inside this tab
    overview_group = next(
        (
            g
    ...
    # Replace pages under "Commands" with generated list
